ad_library {
    ...
    API for the q-tables package
    @creation-date 12 Oct 2016
    @cs-id $Id:
}

# Migrated procs from temporary location in spreadsheet package
# renamed qss_tips_* to qt_*

ad_proc -private qt_user_id_set {
} {
...
ad_proc -public qt_field_id_name_list {
    table_id
} {
    ...
    if {[qf_is_natural_number $table_id ]} {
        set db_sql {
            select id,name from qt_field_defs
            where instance_id=:instance_id
            and table_id=:table_id
            and trashed_p!='1'}
        set fields_lists [db_list_of_lists qt_field_defs_id_name_r $db_sql]
...
ad_proc -public qt_field_label_name_list {
    table_id
} {
    ...
    if {[qf_is_natural_number $table_id ]} {
        set db_sql {select label,name from qt_field_defs
            where instance_id=:instance_id
            and table_id=:table_id
            and trashed_p!='1'}
        set fields_lists [db_list_of_lists qt_field_defs_label_name_r $db_sql]
        foreach row $fields_lists {
            foreach {label name} {
...
ad_proc -private qt_field_defs_maps_set {
    table_id
    ...
    upvar 1 instance_id instance_id
    set fields_lists [qt_field_def_read $table_id $filter_by_label_list]
    ns_log Notice "qt_field_defs_maps_set.96: fields_lists '${fields_lists}'"
    ...
    if { [llength $fields_lists ] > 0 } {
        ...
        if { $set_field_type_label_arr_p } {
            ...
            ns_log Notice "qt_field_defs_maps_set.137: field_label_type_list '${field_label_type_list}'"
        }
        if { $set_field_id_label_arr_p } {
            array set field_id_label_arr $field_label_id_list
            ns_log Notice "qt_field_defs_maps_set.140: field_label_id_list '${field_label_id_list}'"
        }
        if { $set_field_type_id_arr_p } {
            array set field_type_id_arr $field_id_type_list
            ns_log Notice "qt_field_defs_maps_set.145: field_id_type_list '${field_id_type_list}'"
        }
        if { $set_field_label_id_arr_p } {
            array set field_label_id_arr $field_id_label_list
            ns_log Notice "qt_field_defs_maps_set.140: field_id_label_list '${field_id_label_list}'"
        }
    }
...
ad_proc -public qt_table_id_of_label {
    table_label
} { 
    ...
    set table_id ""
    set db_sql {
        select id as table_id from qt_table_defs
        where label=:table_label
        and instance_id=:instance_id
        and trashed_p!='1'}
    db_0or1row qt_table_defs_r_name_untrashed $db_sql
    return $table_id
}

ad_proc -private qt_table_id_exists_q {
    table_id
    {trashed_p "0"}
} {
    ...
    if { ![qf_is_true $trashed_p ] } {
        set exists_p [db_0or1row qt_trashed_table_id_exists {
            select id from qt_table_defs 
            where id=:table_id 
            and instance_id=:instance_id limit 1
        } ]
    } else {
        set exists_p [db_0or1row qt_untrashed_table_id_exists {
            select id from qt_table_defs 
            where id=:table_id
            and instance_id=:instance_id
...
ad_proc -private qt_field_def_id_exists_q {
    field_id
    table_id
    {trashed_p "0"}
} {
    ...
    upvar 1 instance_id instance_id
    if { ![qf_is_true $trashed_p ] } {
        set exists_p [db_0or1row qt_trashed_field_id_exists {
            select id from qt_field_defs
            where id=:field_id 
            and table_id=:table_id
            and instance_id=:instance_id limit 1
        } ]
    } else {
        set exists_p [db_0or1row qt_untrashed_field_id_exists {
            select id from qt_field_defs
            where id=:field_id
            and table_id=:table_id
            and instance_id=:instance_id
...
ad_proc -private qt_row_id_exists_q {
    row_id
    table_id
    {trashed_p "0"}
} {
    ...
    upvar 1 instance_id instance_id
    if { [qf_is_true $trashed_p ] } {
        set exists_p [db_0or1row qt_trashed_row_id_exists {
            select row_id from qt_field_values
            where row_id=:row_id
            and table_id=:table_id
            and instance_id=:instance_id limit 1} ]
    } else {
        set exists_p [db_0or1row qt_untrashed_row_id_exists {
            select row_id from qt_field_values
            where row_id=:row_id
            and table_id=:table_id
            and instance_id=:instance_id
...
ad_proc -public qt_table_def_read {
    table_label
} { 
    ...
    upvar 1 instance_id instance_id
    set table_list [list ]
    set db_sql {select id,label,name,flags,trashed_p from qt_table_defs
        where label=:table_label
        and instance_id=:instance_id
        and trashed_p!='1'}
    set exists_p [db_0or1row qt_table_defs_r1_untrashed $db_sql]
    if { $exists_p } {
...
ad_proc -public qt_table_def_read_by_id {
    table_id
} { 
    Returns list of table_id, label, name, flags, trashed_p or empty list if not found.
} {
...
ad_proc -public qt_table_def_create {
    label
    name
    {flags ""}
} {
    ...
    if { [hf_are_printable_characters_q $label] && [hf_are_visible_characters_q $name] } {
        set existing_id [qt_table_id_of_label $label]
        ...
        if { $existing_id eq "" } {
            set id [db_nextval qt_id_seq]
            set trashed_p "0"
            db_dml qt_table_cre {
                insert into qt_table_defs 
                (instance_id,id,label,name,flags,user_id,created,trashed_p)
                values (:instance_id,:id,:label,:name,:flags,:user_id,now(),:trashed_p)                   
            }
        } else {
            ns_log Notice "qt_table_def_create.273: table label '${label}' already exists."
        }
    } else {
        ns_log Notice "qt_table_def_create.276: table label or name includes characters not allowed."
    }
    return $id
}


ad_proc -public qt_table_def_update {
    table_id
    args
} {
    ...
    upvar 1 instance_id instance_id
    set exists_p [db_0or1row qt_table_def_ur {
        select label,name,flags from qt_table_defs 
        where instance_id=:instance_id 
        and id=:table_id
        and trashed_p!='1'}]
    if { $exists_p } {
...
ad_proc -public qt_table_def_trash {
    table_id
} {
    ...
    upvar 1 instance_id instance_id
    qt_user_id_set
    set success_p [qt_table_id_exists_q $table_id]
    if { $success_p } {
        db_dml qt_table_trash {
            update qt_table_defs 
...
ad_proc -public qt_table_read_as_array {
    name_array
    table_label
    ...
    set table_id [qt_table_id_of_label $table_label]
    set success_p 0
    
    if { [qf_is_natural_number $table_id] } {
        set count [qt_field_defs_maps_set $table_id "" field_id_arr type_arr label_arr field_ids_list field_labels_list]
        if { $count > 0 } {
            set row_ids_sql ""
            if { $row_id_list ne "" } {
                ...
                if { [hf_natural_number_list_validate $row_id_list] } {
                    ...
                    ns_log Warning "qt_read.31: One or more row_id are not a natural number '${row_id_list}'"
                    set row_ids_sql "na"
                }
            }
            set vc1k_search_sql ""
                ...
                foreach {label vc1k_search_val} $vc1k_search_lv_list {
                    ...
                    if { [info exists field_id_arr(${label}) ] && $vc1k_search_sql ne "na" } {
                        ...
                        if { $vc1k_search_val eq "" } {
                            ...
                            append vc1k_search_sql "select row_id from qt_field_values
    ...
    where table_id=:table_id 
        ...
        and field_id='" 
                        ...
                        ns_log Warning "qt_read.492: no field_id for search label '${label}' \
                ...
                foreach cell_list $values_by_row_lists {
                    foreach {row_id field_id f_vc1k f_nbr f_txt} $cell_list {
                        ...
                        if { [info exists type_arr(${field_id}) ] } {
                            ...
                            set v [qt_value_of_field_type $type_arr(${field_id}) $f_nbr $f_txt $f_vc1k]
                        } else {
                            ns_log Warning "qt_read.54: field_id does not have a field_type. \
...
ad_proc -public qt_table_read {
    ...
    if { [qf_is_natural_number $table_id] } {
        set label_ids_list_len [qt_field_defs_maps_set $table_id "" field_id_arr type_arr label_arr label_ids_list labels_list]
    ...
    where table_id=:table_id 
        ...
        and field_id='" 
                        ...
                        ns_log Warning "qt_read.571: no field_id for search label '${label}' \
 table_label '${table_label}' "
                        set vc1k_search_sql "na"
                    }
                }
            ...
            if { $row_ids_sql eq "na" || $vc1k_search_sql eq "na" } {
                ...
                set db_sql "\
    select row_id, field_id, f_vc1k, f_nbr, f_txt from qt_field_values \
    where table_id=:table_id \
        and instance_id=:instance_id \
        and trashed_p!='1' ${vc1k_search_sql} ${row_ids_sql} order by row_id, field_id asc"
                set values_lists [db_list_of_lists qt_field_values_r_sorted $db_sql]
                ...
                foreach cell_list $values_lists {
                    foreach {row_id field_id f_vc1k f_nbr f_txt} $cell_list {
                        ...
                        if { ![qf_is_natural_number $field_id] || ![qf_is_natural_number $current_field_id] } {
                            ns_log Warning "qt_table_read.754: field_id '${field_id} current_field_id '${current_field_id}' This should not happen."
                        }
...
ad_proc -public qt_field_def_create {
    args
} {
    ...
    if { $success_p } {
        ...
        set trashed_p 0
        if { [qf_is_natural_number $table_id] } {
            set new_id [db_nextval qt_id_seq]
            db_dml qt_field_def_cr {insert into qt_field_defs
...
ad_proc -public qt_field_def_trash {
    field_ids
    table_id
} {
    ...
    foreach field_id $field_ids_list {
        set success_p [qt_field_def_id_exists_q $field_id $table_id]
        set success_p_tot [expr { $success_p && $success_p_tot } ]
        if { $success_p } {
            db_dml qt_field_trash_def1 {
                update qt_field_defs 
                set trashed_p='1',trashed_by=:user_id,trashed_dt=now()
                where id=:field_id
                and table_id=:table_id
                and instance_id=:instance_id}
...
ad_proc -public qt_field_def_update {
    table_id
    args
} {
    ...
    if { $includes_ref_p && $includes_set_p } {
        ...
        set db_sql "select id as field_id,label,name,default_val,tdt_data_type,\
                        field_type,created as c_date,user_id as c_user_id from qt_field_defs \
                    where instance_id=:instance_id \
                        and table_id=:table_id \
                        and trashed_p!='1' ${extra_ref_sql}" 
        set exists_p [db_0or1row qt_field_def_r_u1 $db_sql]
        if { $exists_p } {
            qt_user_id_set
            ...
            set trashed_p 0
            db_transaction {
                db_dml qt_field_def_u1 { update qt_field_defs 
                    set trashed_p='1',
                    trashed_dt=now(),
                    trashed_by=:user_id
                    where id=:field_id 
                    and instance_id=:instance_id 
                    and table_id=:table_id }
                db_dml qt_field_def_u1_cr {
                    insert into qt_field_defs
...
ad_proc -private qt_field_def_read {
    table_id
    {field_labels ""}
    {field_ids ""}
} { 
    ...
    if {[qf_is_natural_number $table_id ]} {
        set db_sql {
            select id as field_id,label,name,default_val,tdt_data_type,field_type from qt_field_defs
            where instance_id=:instance_id
            and table_id=:table_id
            and trashed_p!='1'}
        set fields_lists [db_list_of_lists qt_field_defs_r $db_sql]
        ...
        #ns_log Notice "qt_field_def_read.790 field_label_list '${field_label_list}' 
        ...
        #ns_log Notice "qt_field_def_read.808 field_id_list '${field_id_list}' 
...
ad_proc -public qt_row_create {
    ...
    set new_id ""
    if { [qf_is_natural_number $table_id] } {
        set count [qt_field_defs_maps_set $table_id t_arr l_arr "" "" "" field_labels_list]
            ...
            db_transaction {
                foreach {label value} $label_value_list {
                    ...
                    if { $label in $field_labels_list && $value ne "" } {
                        ...
                        set trashed_p 0
                        qt_set_by_field_type $field_type $value f_nbr f_txt f_vc1k
                        ns_log Notice "qt_row_create.911: field_type '${field_type}' \
 value '${value}' f_nbr '${f_nbr}' f_txt '${f_txt}' f_vc1k '${f_vc1k}'"
                        set db_sql {
                            insert into qt_field_values
                            ...
                            values (:instance_id,:table_id,:new_id,:trashed_p,now(),
                                    :user_id,:field_id,:f_vc1k,:f_nbr,:f_txt) }
                        db_dml qt_field_values_row_cr_1f $db_sql
                    }
                }
            }
        } else {
            ns_log Notice "qt_row_create.908: No fields defined for table_id '${table_id}'."
        }
    } else {
        ns_log Notice "qt_row_create.911: table_id '${table_id}' not a valid number."
    }
    return $new_id
}

ad_proc -private qt_value_of_field_type {
    field_type
    f_nbr
    f_txt
    f_vc1k
    ...
    switch -exact -- $field_type {
        ...
        default {
            set v [qal_first_nonempty_in_list [list $f_nbr $f_vc1k $f_txt]]
            ns_log Warning "qt_value_of_field_type.843: unknown field_type '${field_type}'. \
...
ad_proc -private qt_set_by_field_type {
    field_type
    value
    nbr_var_name
    txt_var_name
    ...
    switch -exact -- $field_type {
        ...
        default {
            ns_log Warning "qt_set_by_field_type.783: field_type '${field_type}' not valid. \
 Defaulting to txt"
            set f_nbr ""
            set f_txt $value
            set f_vc1k ""
    ...
    ns_log Notice "qt_set_by_field_type.984: field_type '${field_type}' value '${value}' \
 f_nbr '${f_nbr}' f_txt '${f_txt}' f_vc1k '${f_vc1k}'"
    return $success_p
}


ad_proc -public qt_row_update {
    table_id
    row_id
    label_value_list
} {
    ...
    if { [qf_is_natural_number $table_id] && [qf_is_natural_number $row_id ] } {
        set success_p [qt_row_id_exists_q $row_id $table_id ]
        if { $success_p } {
            set count [qt_field_defs_maps_set $table_id t_arr l_arr "" "" "" field_labels_list ]
            if { $count > 0 } { 
                qt_user_id_set
                db_transaction {
                    foreach {label value} $label_value_list {
                        if { $label in $field_labels_list } {
                            #set field_id $l_arr(${label})
                            #set field_type $t_arr(${label})
                            ns_log Notice "qt_row_update.1027 table_id '${table_id}' \
 row_id '${row_id}' label '${label}' t_arr(${label}) '$t_arr(${label})'"
                            qt_set_by_field_type $t_arr(${label}) $value f_nbr f_txt f_vc1k
                            qt_cell_update $table_id $row_id $l_arr(${label}) $value
                        } else {
                            ns_log Notice "qt_row_update.1031 label '${label}' \
 not in table_id '${table_id}'. update to value '${value}' ignored."
        ...
        ns_log Warning "qt_row_udpate.1035: table_id '${table_id}' \
...
ad_proc -public qt_row_of_table_label_value {
    table_id
    {vc1k_search_label_val_list ""}
    {if_multiple "1"}
    {row_id_var_name ""}
    ...
    if { [qf_is_natural_number $table_id] } {
        ...
        set label_ids_list_len [qt_field_defs_maps_set $table_id "" field_id_arr type_arr label_arr field_ids_list ""]
        if { $label_ids_list_len > 0 } {
            set vc1k_search_sql ""
            set sort_sql ""
            ...
            if { $vc1k_search_label_val_list ne "" } {
                # search scope
                set vc1k_search_lv_list [qf_listify $vc1k_search_label_val_list]
                ns_log Notice "qt_row_of_table_label_value.1056: vc1k_search_label_val_list  \
 '${vc1k_search_label_val_list}' vc1k_search_lv_list '${vc1k_search_lv_list}'"
    ...
    where table_id=:table_id \
        ...
        and field_id='" 
                        ...
                        ns_log Warning "qt_row_of_table_label_value.1067: no field_id \
 for search label '${label}' table_id '${table_id}' "
            ...
            if { $vc1k_search_sql eq "na" } {
                ...
                # get row id, then row
                ns_log Notice "qt_row_of_table_label_value.1084: \
 vc1k_search_sql '${vc1k_search_sql}' sort_sql '${sort_sql}'"
    ...
    where instance_id=:instance_id \
        ...
        and trashed_p!='1' ${vc1k_search_sql} ${sort_sql}"
                set row_ids_list [db_list qt_field_values_row_id_search $db_sql]
                ...
                if { $exists_p && $if_multiple eq "-1" } {
                    ...
                    if { [llength $row_ids_unique_list] > 1 } {
                        ns_log Notice "qt_row_of_table_label_value.1094: Rejecting row_id,\
 because if_multiple=-1: row_ids_list '${row_ids_list}' row_ids_unique_list '${row_ids_unique_list}'"
                ...
                if { $exists_p } {
                    ...
                    set db_sql {
                        ...
                        and instance_id=:instance_id
                        and trashed_p!='1'}
                    set values_lists [db_list_of_lists qt_field_values_r1m $db_sql]
                    ...
                    foreach row $values_lists {
                        foreach {field_id row_id f_vc1k f_nbr f_txt} $row {
                            if { [info exists type_arr(${field_id}) ] } {
                                ...
                                ns_log Warning "qt_row_of_table_label_value.1092: field_id \
 does not have a field_type. table_id '${table_id}' field_id '${field_id}' row_id '${row_id}'"
                    ...
                    ns_log Notice "qt_row_of_table_label_value.1099: row not found \
 for search '${vc1k_search_label_val_list}'."
                }
            }
        } else {
            ns_log Notice "qt_row_of_table_label_value.1101: no fields defined for table_id '${table_id}'"
        }
    } else {
        ns_log_ Notice "qt_row_of_table_label_value.1104: table_id '${table_id}' not a natural number."
    }
    return $row_list
}

ad_proc -public qt_rows_read {
    table_id
    row_ids_list
} {
    ...
    if { [qf_is_natural_number $table_id] && [hf_natural_number_list_validate $row_ids_list] } {
        set count [qt_field_defs_maps_set $table_id "" "" type_arr label_arr "" labels_list]
        if { $count > 0  } {
            lappend rows_lists $labels_list
            set db_sql "select field_id, row_id, f_vc1k, f_nbr, f_txt from qt_field_values \
                where table_id=:table_id \
                and instance_id=:instance_id \
                and trashed_p!='1' \
                and row_id in ([template::util::tcl_to_sql_list $row_id_list])"
            set values_lists [db_list_of_lists qt_field_values_r_mult $db_sql]
            ...
            foreach row $values_lists {
                foreach {field_id row_id f_vc1k f_nbr f_txt} $row {
                    if { [info exists type_arr(${field_id}) ] } {
                        ...
                        ns_log Warning "qt_read_from_id.848: field_id does not have a field_type. \
 table_label '${table_label}' field_id '${field_id}' row_id '${row_id}'"
                    }
...
ad_proc -public qt_row_read {
    ...
    set row_list [list ]
    if { [qf_is_natural_number $table_id ] } {
        set count [qt_field_defs_maps_set $table_id "" "" type_arr label_arr field_ids_list ]
        if { $count > 0 } {
            set db_sql {
                select field_id, row_id, f_vc1k, f_nbr, f_txt from qt_field_values 
                where table_id=:table_id
                and row_id=:row_id
                and instance_id=:instance_id
                and trashed_p!='1'}
            ...
            foreach row $values_lists {
                foreach {field_id row_id f_vc1k f_nbr f_txt} $row {
                    if { [info exists type_arr(${field_id}) ] } {
                        ...
                        ns_log Warning "qt_row_read.848: field_id does not have a field_type. \
 table_label '${table_label}' field_id '${field_id}' row_id '${row_id}'"
                    }
                    # label $label_arr(${field_id})
                    lappend row_list $label_arr(${field_id}) $v
...
ad_proc -public qt_row_trash {
    ...
    if { $success_p } {
        qt_user_id_set
        db_dml qt_field_values_row_trash {
            update qt_field_values
            set trashed_p='1',trashed_by=:user_id,trashed_dt=now()
            where row_id=:row_id
            and table_id=:table_id
            and instance_id=:instance_id
...
ad_proc -public qt_cell_read {
    ...
    <br>
    @see qt_row_of_table_label_value 
} {
    upvar 1 instance_id instance_id
    ...
    if { $return_val_label_list_len > 0 } {
        set table_id [qt_table_id_of_label $table_label]
        if { $table_id ne "" } {
            set label_value_list [qt_row_of_table_label_value $table_id $vc1k_search_label_val_list $if_multiple row_id]
            ...
            ns_log Notice "qt_cell_read.1327: table_label not found '${table_label}'"
        }
    } else {
        ns_log Notice "qt_cell_read.1329: No cell labels requested; \
...
ad_proc -private qt_cell_id_exists_q {
    table_id
    row_id
    field_id
} {
    ...
    upvar 1 instance_id instance_id
    set db_sql {
        select f_vc1k, f_nbr, f_txt from qt_field_values
        where row_id=:row_id
        and field_id=:field_id
        and table_id=:table_id
        and instance_id=:instance_id
        and trashed_p!='1'}
    set exists_p [db_0or1row qt_field_values_c1_by_id $db_sql]
    return $exists_p
}

ad_proc -public qt_cell_read_by_id {
    table_id
    row_id
    field_id_list
} {
    ...
    if { [hf_natural_number_list_validate $field_id_list] } {
        set field_id_list_len [llength $field_id_list]
        set db_sql "\
    select field_id,f_vc1k,f_nbr,f_txt from qt_field_values \
    where row_id=:row_id \
        and table_id=:table_id \
        and instance_id=:instance_id \
        and trashed_p!='1' \
    and field_id in ([template::util::tcl_to_sql_list $field_id_list]) "
        set field_id_values_lists [db_list_of_lists qt_cell_read_by_id $db_sql]
        ns_log Notice "qt_cell_read_by_id field_id_values_lists '${field_id_values_lists}'"
        foreach row_list $field_id_values_lists {
            foreach {field_id f_vc1k f_nbr f_txt} $row_list {
                ...
                ns_log Notice "qt_cell_read_by_id.1384 field_id '$field_id' field_value '${field_value}'"
            }
        }
        ns_log Notice "qt_cell_read_by_id.1387 field_id_list '${field_id_list}'"
        foreach field_id $field_id_list {
            set field_value ""
            ns_log Notice "qt_cell_read_by_id.1390: field_id '${field_id}'"
            if { [info exists v_arr(${field_id}) ] } {
                lappend return_value_list $v_arr(${field_id})
            } else {
                ns_log Notice "qt_cell_read_by_id.1394: field_id '${field_id}' \
 not found for row '${row_id}'"
                lappend return_value_list ""
            }
        }
        ns_log Notice "qt_cell_read_by_id.1396 return_value_list '${return_value_list}'"
    } else {
        ns_log Notice "qt_cell_read_by_id.1395 field_id_list did not validate \
...
ad_proc -public qt_cell_update {
    ...
    set exists_p [db_0or1row qt_field_def_read_ft {
        select field_type from qt_field_defs
        where instance_id=:instance_id
        and table_id=:table_id
        and id=:field_id
        and trashed_p!='1'}]
    if { $exists_p } {
        ...
        qt_set_by_field_type $field_type $new_value f_nbr f_txt f_vc1k
        qt_user_id_set
        set trashed_p 0
        db_transaction {
            set success_p [qt_cell_trash $table_id $row_id $field_id ]
            db_dml qt_field_values_row_up_1f { insert into qt_field_values
...
ad_proc -public qt_cell_trash {
    ...
    upvar 1 instance_id instance_id
    set exists_p [qt_cell_id_exists_q $table_id $row_id $field_id]
    if { $exists_p } {
        qt_user_id_set
        db_dml qt_field_values_cell_trash { update qt_field_values
            set trashed_p='1',trashed_by=:user_id,trashed_dt=now()
            where instance_id=:instance_id
            and table_id=:table_id
            and row_id=:row_id
...
ad_proc -public qt_tdt_data_types_to_qdt {
    ...
    Extends qdt::data_types output with modifications from qt_tdt_data_types.
    <br><br>
    ...
    # Mesh tdt_ol data on qdt:data_types' array ie d_arr
        ...
        set new_entry_label ""
        if { $qdt_label in $qdt_labels_list } {
            # Make a copy of qdt_label into new datatype type_name
            foreach qf $qdt_fields_list {
##code was tdt_new_ol
                set d_new_arr(${qdt_label},${qf}) $d_arr(${qdt_label},${qf})
            }
            lappend qdt_overrides_list $qdt_label
            set new_entry_label $qdt_label
        } else {
            if { $type_name in $qdt_labels_list } {
                # Treat type_name as qdt datatype to modify
                #
                foreach qf $qdt_fields_list {
                    set d_new_arr(${type_name},${qf}) $d_arr(${qdt_label},${qf})
                }
                lappend qdt_overrides_list $type_name
                set new_entry_label ""
            }
        }
        if { $new_entry_label ne "" } {

            # overlay with tdt values
            set d_new_arr(${new_entry_label},form_tag_attrs) $form_tag_attrs
            set d_new_arr(${new_entry_label},empty_allowed_p) $empty_allowed_p
            # Delay adding new entries to d_arr
 ...
 label '${new_entry_label}'"
    ...
    if { $tdt_ol_retry_lists_len > 0 } {
        ...
        ns_log Notice "qt_tdt_data_types_to_qdt.1956: Re-trying loop \
 to fit d_arr names '[array names d_arr]'"
            ...
            foreach tdt_ol $tdt_ol_retry_lists {
               ...
               # Mesh tdt_ol data on qdt:data_types' array ie d_arr
                lassign $tdt_ol \
                    ...
                    default_field_type \
                    empty_allowed_p
                ns_log Notice "qt_tdt_data_types_to_qdt.1960: maybe process \
 type_name '${type_name}' qdt_label '${qdt_label}' tdt_new_ol '${tdt_new_ol}'"

                # type_name is new datatype based on datatype with qdt_label
                set tdt_new_ol [list ]
                if { $qdt_label in $qdt_labels_list } {
##code recoding stopped here.
                    # Make a copy of qdt_label into new datatype type_name
                    set tdt_new_ol $d_arr(${qdt_label})
                    lappend qdt_overrides_list $qdt_label
                    ns_log Notice "qt_tdt_data_types_to_qdt.1979: done for \
 qdt_label '${qdt_label}'"
                } else {
                    if { $type_name in $qdt_names_list } {
                        ...
                        lappend qdt_overrides_list $type_name
                        ns_log Notice "qt_tdt_data_types_to_qdt.1987: done for \
 type_name '${type_name}'"
                    }
                }
                if { [llength $tdt_new_ol] > 1 } {
                    ...
                    ns_log Notice "qt_tdt_data_types_to_qdt.1996: value check \
 form_tag_attrs '${form_tag_attrs}' empty_allowed_p '${empty_allowed_p}'"
